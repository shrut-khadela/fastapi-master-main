...
from src.user.models import User, Table as TableModel, Menu as MenuModel, Category as CategoryModel, Order as OrderModel, OrderStatus as OrderStatusModel, Stock as StockModel, Invoice as InvoiceModel, PaymentStatus as PaymentStatusModel, Payment as PaymentModel, QRCode as QRCodeModel
...
order_status_crud = CRUDBase[OrderStatusModel, OrderStatusUpdate, OrderStatusResponse](OrderStatusModel)
...
payment_status_crud = CRUDBase[PaymentStatusModel, PaymentStatus, PaymentStatus](PaymentStatusModel)
...
def signup(user_req: SignupRequest, db: get_db):
    ...
    user = user_crud.create(
        db,
        obj_in=UserBase(id=user_id, created_by=user_id, updated_by=user_id, **data),
    )
...
def login(login_creds: LoginRequest, db: get_db):
    ...
    if not user.verify_password(password):
        logger.debug("Login 401: password mismatch for email=%s (DB may use Argon2 - pip install 'passlib[argon2]')", email)
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid credentials"
        )
    return Token(user=user, token=user.create_token())
...
def me(user_db: User = Depends(authenticated_user)):
    return user_db
...
def get_users(db: get_db, page: int = 1, per_page: int = 10):
    ...
    return [UserResponse(id=str(u.id), email=u.email, firstname=u.firstname, lastname=u.lastname, role=u.role) for u in users]


@user_router.put("/user", response_model=UserResponse, status_code=status.HTTP_201_CREATED)
def update_user(user_req: UserUpdate, db: get_db, user_db: User = Depends(authenticated_user)):
    return user_crud.update(db, db_obj=user_db, obj_in=user_req)


@user_router.delete("/user", status_code=status.HTTP_204_NO_CONTENT)
def delete_user(db: get_db, user_db: User = Depends(authenticated_user)):
...
# Table APIs
########################################################
@table_router.post("/create_table", response_model=Table, status_code=status.HTTP_201_CREATED)
def create_table(table_data: Table, db: get_db, user_db: User = Depends(authenticated_user)):
    obj_in = table_data.model_dump()
    obj_in["created_by"] = str(user_db.id)
    obj_in["updated_by"] = str(user_db.id)
...
def update_table(table_id: str, table_data: Table, db: get_db, user_db: User = Depends(authenticated_user)):
    db_table = table_crud.get(db, id=table_id)
    if not db_table:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Table not found"
        )
    db_table.table_no = table_data.table_no
    db_table.updated_by = user_db.id
    db.commit()
    db.refresh(db_table)
    return Table(table_id=str(db_table.id), table_no=db_table.table_no)


@table_router.delete("/delete_table_by_id/{table_id}", status_code=status.HTTP_204_NO_CONTENT)
...
@category_router.post("/create_category", response_model=Category, status_code=status.HTTP_201_CREATED)
def create_category(category_data: Category, db: get_db, user_db: User = Depends(authenticated_user)):
    obj_in = category_data.model_dump()
    obj_in["created_by"] = str(user_db.id)
    obj_in["updated_by"] = str(user_db.id)
    category_crud.create(db, obj_in=obj_in)
    return Category(category_id=str(category_data.category_id), category_name=category_data.category_name)
...
@menu_router.post("/create_menu", response_model=Menu, status_code=status.HTTP_201_CREATED)
def create_menu(menu_data: Menu, db: get_db, user_db: User = Depends(authenticated_user)):
    obj_in = menu_data.model_dump()
    obj_in["created_by"] = str(user_db.id)
    obj_in["updated_by"] = str(user_db.id)
    # DB stores item_list and category_name as strings (JSON)
    obj_in["item_list"] = json.dumps(obj_in["item_list"], default=str)
    obj_in["category_name"] = json.dumps(obj_in["category_name"], default=str)
...
def update_menu(menu_id: str, menu_data: Menu, db: get_db, user_db: User = Depends(authenticated_user)):
    menu_id = _normalize_menu_id(menu_id)
    menu = (
        db.query(MenuModel)
        .filter(MenuModel.menu_id == menu_id, MenuModel.is_deleted == false())
    ...
    menu.updated_by = str(user_db.id)
    db.add(menu)
    db.commit()
    db.refresh(menu)
    return _menu_row_to_schema(menu)

@menu_router.delete("/delete_menu_by_id/{menu_id}", status_code=status.HTTP_204_NO_CONTENT)
...
def _order_status_for_response(o: OrderModel, db) -> OrderStatus:
    ...
    row = (
        ...
        .filter(OrderStatusModel.order_id == str(o.id), OrderStatusModel.is_deleted == false())
        .first()
    )
...
def create_order(order_data: OrderCreate, db: get_db, user_db: User = Depends(authenticated_user)):
    try:
    ...
    obj_in = {
        ...
        "order_done": "false",
        "order_cancel": "false",
        "created_by": str(user_db.id),
        "updated_by": str(user_db.id),
    }
...
def update_order(order_id: str, order_data: OrderUpdate, db: get_db, user_db: User = Depends(authenticated_user)):
    ...
    order.updated_by = str(user_db.id)
    order_crud.update(db, db_obj=order, obj_in=order_data)
    return _order_row_to_response(order, db)

@order_router.delete("/delete_order_by_id/{order_id}", status_code=status.HTTP_204_NO_CONTENT)
...
@order_status_router.put("/update_order_status/{order_id}", response_model=OrderStatusResponse)
def update_order_status(order_id: str, order_status_data: OrderStatusUpdate, db: get_db, user_db: User = Depends(authenticated_user)):
    order_status = (
        db.query(OrderStatusModel)
        .filter(OrderStatusModel.order_id == order_id, OrderStatusModel.is_deleted == false())
        .first()
    )
    if not order_status:
    ...
    order_status.updated_by = str(user_db.id)
    order_status_crud.update(db, db_obj=order_status, obj_in=order_status_data)
    db.refresh(order_status)
    return OrderStatusResponse(order_id=str(order_status.order_id), status=order_status.status or "")

########################################################
# Stock APIs
########################################################

@stock_router.post("/create_stock", response_model=Stock, status_code=status.HTTP_201_CREATED)
def create_stock(stock_data: StockCreate, db: get_db, user_db: User = Depends(authenticated_user)):
    ...
    return Stock(id=str(created.id), name=created.name, quantity=created.quantity, unit_of_measure=created.unit_of_measure, cost_price=created.cost_price)
...
@invoice_router.post("/create_invoice", response_model=Invoice, status_code=status.HTTP_201_CREATED)
def create_invoice(invoice_data: InvoiceCreate, db: get_db, user_db: User = Depends(authenticated_user)):
    ...
    try:
        ...
        raise
    return Invoice(invoice_id=str(created.id), order_id=created.order_id, invoice_number=created.invoice_number, invoice_date=created.invoice_date, total_amount=created.total_amount, payment_status=created.payment_status, notes=created.notes)
...
def get_invoices(db: get_db, page: int = 1, per_page: int = 10):
    ...
    return [Invoice(invoice_id=str(i.id), order_id=i.order_id, invoice_number=i.invoice_number, invoice_date=i.invoice_date, total_amount=i.total_amount, payment_status=i.payment_status, notes=i.notes) for i in invoices]
...
def get_invoice(invoice_id: str, db: get_db):
    ...
    return Invoice(invoice_id=str(invoice.id), order_id=invoice.order_id, invoice_number=invoice.invoice_number, invoice_date=invoice.invoice_date, total_amount=invoice.total_amount, payment_status=invoice.payment_status, notes=invoice.notes)

@invoice_router.put("/update_invoice/{invoice_id}", response_model=Invoice)
def update_invoice(invoice_id: str, invoice_data: InvoiceUpdate, db: get_db, user_db: User = Depends(authenticated_user)):
    invoice = invoice_crud.get(db, id=invoice_id)
    if not invoice:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Invoice not found")
    ...
    invoice.updated_by = str(user_db.id)
    invoice_crud.update(db, db_obj=invoice, obj_in=invoice_data)
    db.refresh(invoice)
    return Invoice(invoice_id=str(invoice.id), order_id=invoice.order_id, invoice_number=invoice.invoice_number, invoice_date=invoice.invoice_date, total_amount=invoice.total_amount, payment_status=invoice.payment_status, notes=invoice.notes)

@invoice_router.delete("/delete_invoice_by_id/{invoice_id}", status_code=status.HTTP_204_NO_CONTENT)
...
# def update_payment_status(payment_status_id: str, payment_status_data: PaymentStatusUpdate, db: get_db, user_db: User = Depends(authenticated_user)):
#     payment_status = (
...
def generate_upi_uri(order_id: str, amount: float) -> str:
    """Build UPI payment link so when customer scans & pays, money credits to your bank (UPI_ID)."""
    import urllib.parse

    from src.config import Config

    vpa = (Config.UPI_ID or "").strip()
    if not vpa:
        raise ValueError(
            "UPI_ID is not set. Add your UPI ID to .env (e.g. UPI_ID=9876543210@ybl or yourname@paytm) "
            "so payment QR codes credit to your bank account."
        )

    params = {
        "pa": vpa,  # Payee VPA – your UPI ID (money goes here)
        "pn": Config.UPI_MERCHANT_NAME,
...
@payment_router.post("/create_payment", response_model=PaymentResponse,status_code=status.HTTP_201_CREATED)
def create_payment(request: Request, payload: PaymentCreate, db: get_db):
    ...
    db.commit()
    db.refresh(payment)
    # Generate first QR (linked to your UPI_ID so payment credits to your account)
    try:
...
def payment_webhook(payload: PaymentWebhook, db: get_db):
    ...
    if payload.payment_id:
        payment = (
            db.query(PaymentModel)
            .filter(PaymentModel.id == payload.payment_id)
            .first()
        )
    else:
        payment = (
    ...
    if payment.status == PaymentStatus.PAID:
        return {"ok": True, "payment_id": str(payment.id), "payment_status": "paid", "message": "Already paid"}
...
@payment_router.get(
    ...
    responses={200: {"content": {"image/png": {}}, "description": "QR code image (PNG)"}},
)
def get_payment_qr_image(
    ...
    png_bytes = generate_qr_png(qr.qr_data, size=min(max(size, 1), 20), border=min(max(border, 0), 10))
...
def mark_payment_paid(
    ...
    _set_payment_paid_and_persist(db, payment, upi_ref_id=body.upi_ref_id if body else None)
